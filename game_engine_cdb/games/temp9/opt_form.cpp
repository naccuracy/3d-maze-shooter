...
#include<string>
using namespace std;
OptForm::OptForm(InterfaceItem *it):Form(it)
{
    ...
    cancelBtn->SetCaption("Отмена");
    cancelBtn->Show();

    lengthLabel=new TextLabel(this);
    lengthLabel->caption="Длина";
    lengthLabel->Show();
...
int OptForm::OnMouseDown(int x, int y, int btn)
{
    ...
    {
        if(childs[child_focus]==saveBtn)SaveClick();
        if(childs[child_focus]==cancelBtn)CancelClick();
    }
    return res;
}
...
void OptForm::SaveClick()
{
    ...
    statusLabel->caption="Настройки успешно сохранены, перезагрузите игру";
    return;
}
