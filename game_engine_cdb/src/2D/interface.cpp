...
InterfaceItem::~InterfaceItem()
{
 ...
 childs.clear();
 //msg((char*)caption.c_str());
}

InterfaceItem* InterfaceItem::GetParent()
{
 return parent;
}
...
void InterfaceItem::RedrawItem()
{
 ...
 {
     n=i;
     if(n!=child_focus && childs[i]->visible)childs[i]->RedrawItem();
 }

 if(child_focus>=0)childs[child_focus]->RedrawItem();
 return;
}
...
int InterfaceItem::OnType(int ch)
{
 ...
 if(focus)
    for(i=0;i<n;i++)
    {
     if(i==child_focus && childs[i]->focus)childs[i]->OnType(ch);
    }
 return 0;
}

//передает сообщение потомку, имеющему фокус, если сам элемент находится в фокусе ввода
int InterfaceItem::OnKeyDown(int c)
{
 ...
 if(focus)
    for(i=0;i<n;i++)
    {
     if(i==child_focus && childs[i]->focus)childs[i]->OnKeyDown(c);
    }
 return 0;
}
...
void InterfaceItem::RenderString(string str)
{
    ...
    for(i=0;i<len;i++)
    {
        ...
        if(j!=32 && font->symbols[j].size())
        {
            ...
            for(k=0;k<font->symbols[j].size();k++)
            {
                if(k%2==1)
                {
                    ...
                    py=Y-(int)(font->symbols[j][k-1].y*(float)font_size)+font_size;
        ...
        if(j==32) X+=(int)(font->s_width[32]*(float)font->font_size)+font->aprosh;
        //glPopMatrix();
    }
    glLineWidth(1);
    font->SetSize(fs);
...
//calculate size of string str in pixels
int InterfaceItem::SizeOfString(string str)
{
    int n=str.size();
    int j;
...
//
void TextLabel::SetCaption(string str)
{
  caption=str;
...
//----------------------------------------------------------------------------------------------------
Form::Form(InterfaceItem* it):InterfaceItem(it)
{
}
//
Form::~Form()
...
//-------------------------------------------------------------------------
//constructor
Button::Button(InterfaceItem* it):InterfaceItem(it)
{
    mousebtn=0;
}
//destructor
...
int Menu::OnMouseDown(int x, int y, int btn)
{
 ...
 if(selected)
 {
     ...
     if(btn==1 && child_focus>=0)
        printf("\n%s",childs[child_focus]->caption.c_str());
 }
 return res;
}
...
int Menu::SetStringItems(vector<string> strs)
{
    TextLabel *tl=NULL;
    int fs=font->font_size;
    font->SetSize(font_size);
...
int Message::ShowMessage(string mssg)
{
 caption=mssg;
 float w=SizeOfString(caption);
 float h=font_size*3.0;
...
void ProgressBar::RedrawItem()
{
 if(visible)
 {
    ...
    if(string_flag)
    {
        ...
        temp_str.clear();
        i=(width-str_len)/2;//позиция начала строки
        if(str_len>width)
        {
            n=width/(font->s_width[32]+font->aprosh);
            for(i=0;i<n;i++)temp_str.push_back(caption[i]);
            i=font->aprosh;
            too_big_flag=1;
        }
        else temp_str=caption;
...
void ProgressBar::SetString(string str)
{
 caption=str;
 return;
}
...
{
 if(p>100.0)percent=100.0;
 else percent=p;
 return;
}
//-------------------------------------------------------------------------------------
EditBox::EditBox(InterfaceItem* it):InterfaceItem(it)
{
 caption="";
}
...
{
    font->SetSize(font_size);
    int ml=SizeOfString(caption)+font->aprosh*2+font->s_width[32]*font_size;
...
void ImageLabel::SetImage(int ix)//передается номер уже проинициализированной текстуры))))
{
    tex_index = ix;
}
