...
void Frame::GetCameraMatrix(float res[16])//записывает в переменную res матрицу камеры
{
 Point3f nn=(n*(-1.0));
 res[0]=u[0]; res[4]=u[1]; res[8]=u[2]; res[12]=-(position.dotProduct(u));
 res[1]=v[0]; res[5]=v[1]; res[9]=v[2]; res[13]=-(position.dotProduct(v));
 res[2]=nn[0]; res[6]=nn[1]; res[10]=nn[2]; res[14]=-(position.dotProduct(nn));
 res[3]=0; res[7]=0; res[11]=0; res[15]=1;
 return;
}

void Frame::GetActorMatrix(float res[16])//записывает в переменную res матрицу актера
{
 res[0]=u[0]; res[4]=v[0]; res[8]=n[0]; res[12]=position[0];
 res[1]=u[1]; res[5]=v[1]; res[9]=n[1]; res[13]=position[1];
 res[2]=u[2]; res[6]=v[2]; res[10]=n[2]; res[14]=position[2];
 res[3]=0; res[7]=0; res[11]=0; res[15]=1;
 return;
}
...
void Frame::set(Point3f p, Point3f t, Point3f up)//установка позиции цели и вектора направления вверх
{
 position=p;
 CalculateUVN(p,t,up);
}
...
void Frame::CalculateUVN(Point3f pos, Point3f tag, Point3f up)//расчет системы координат камеры
{
 n=tag-pos;
 n.normalize();
 v=up;
