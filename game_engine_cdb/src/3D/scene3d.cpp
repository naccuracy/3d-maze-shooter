...
Scene3d::Scene3d()
{
    lmAmbient = Point3f(0.2, 0.2, 0.2); lmAmbient.data[3]=1.0;
    vpType = false; //бесконечно удаленная точка наблюдения
    doubleSide = false; //освещается только лицевая сторона
    //lmColorControl = GL_SINGLE_COLOR; //по умолчанию
...
{
    name = "nLight0";
    pos = Point3f(0.0,0.0,1.0); setLocationType(direct_light);
    id = GL_LIGHT0;
    ambient = Point3f(0.0,0.0,0.0); ambient.data[3] = 1.0;
    diffuse = Point3f(1.0,1.0,1.0); diffuse.data[3] = 1.0;
    specular = diffuse;
...
void nLight::setLocationType(float type)
{
    pos.data[3]=type;
}
//настройка openGL
