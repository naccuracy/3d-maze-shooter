...
void SimpleMesh::SaveToCSV(char* fn)
{
    ...
    f=fopen(fn,"wt");
    if(f==NULL)return;
    fprintf(f,"%s\n%d;%d;%d\n",name.c_str(),points.size(),faces.size(),texcoords.size());
...
void SimpleMesh::DrawFlatTexturedReferenece()
{
    ...
    for(i=0;i<nf;i++)
    {
        ...
        glTexCoord2f(texcoords[tex_faces[i].a].data[0],texcoords[tex_faces[i].a].data[1]);
        glVertex4fv(points[faces[i].a].data);
        glTexCoord2f(texcoords[tex_faces[i].b].data[0],texcoords[tex_faces[i].b].data[1]);
        glVertex4fv(points[faces[i].b].data);
        glTexCoord2f(texcoords[tex_faces[i].c].data[0],texcoords[tex_faces[i].c].data[1]);
...
void SimpleMesh::DrawWithTexCoords(int nrm_type)
{
    ...
    for(i=0;i<nf;i++)
    {
        ...
        tc = tex_faces[i].c;
        glNormal3fv(normals[i].data);
        glTexCoord2f(texcoords[ta].data[0],texcoords[ta].data[1]);glVertex4fv(points[a].data);
        glTexCoord2f(texcoords[tb].data[0],texcoords[tb].data[1]);glVertex4fv(points[b].data);
        glTexCoord2f(texcoords[tc].data[0],texcoords[tc].data[1]);glVertex4fv(points[c].data);
    }
    else
    for(i=0;i<nf;i++)
    {
...
void SimpleMesh::DrawSolidReference(Point3f clr, int nrm_type)//рисуем треугольники
{
    unsigned int i;
    glColor4f(clr[0],clr[1],clr[2],clr[3]);//!!!
    unsigned int nf = faces.size();
...
void SimpleMesh::DrawSolid(Point3f clr, int nrm_type)//рисуем треугольники
{
    unsigned int i;
    int a, b, c;
...
void SimpleMesh::Normalize(int sm_param)//обход против часовой стрелки
{
 Point3f ab, ac, n;
...
void SimpleMesh::LoadSphere(float R, int seg, int slice)//сфера в начале координат
{
    ...
    points.push_back(Point3f(0,R,0)); //верхний полюс
    points.push_back(Point3f(0,-R,0));//нижний полюс
    int j1;
    for(i=0;i<slice-2;i++)
    {
        for(j=0;j<seg;j++)
        {
            j1 = j+1;
            if(j1 == seg)j1 = 0;
            faces.push_back(Face3i(i*seg+j, (i+1)*seg+j1, (i+1)*seg+j));
...
void  SimpleMesh::CalcBoundingBox()
{
    ...
    for(i=1;i<n;i++)
    {
        if(min[0] > points[i][0]) min.data[0] = points[i][0];
        if(min[1] > points[i][1]) min.data[1] = points[i][1];
        if(min[2] > points[i][2]) min.data[2] = points[i][2];

        if(max[0] < points[i][0]) max.data[0] = points[i][0];
        if(max[1] < points[i][1]) max.data[1] = points[i][1];
        if(max[2] < points[i][2]) max.data[2] = points[i][2];
    }
    return;
}
